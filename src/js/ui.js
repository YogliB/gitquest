...
export function renderChoices(container, choices, onChoice) {
  ...
  choices.forEach((choice, i) => {
    ...
    // Display 1, 2, 3...
    btn.innerHTML = `<span class="choice-num">${
      i + 1
    }.</span><span class="choice-text">${choice.text}</span>`;
...
export function updateQuestLog(quests) {
  ...
  log.innerHTML = quests
    .slice(0, 5)
    .map((q) => `<div class="quest-entry">Ch.${q.chapter}: ${q.text}</div>`)
    .join("");
}
...
export function updateCommitLore(commit) {
  ...
  lore.innerHTML = `
    ...
    <div class="commit-lore-msg">"${commit.subject.slice(0, 60)}${
...
export function renderSaveSlots(saves, onLoad, onDelete) {
  ...
  if (saves.length === 0) {
    container.innerHTML =
      '<p style="color:var(--color-text-dim);font-size:0.82rem;text-align:center">No saved quests</p>';
    return;
  }
    ...
    <div class="save-slot" data-slot="${s.slot}">
      <div>
        ...
        <div style="font-size:0.72rem;color:var(--color-text-dim)">${
          s.style
        } · Ch.${s.chapter} · ${new Date(s.savedAt).toLocaleDateString()}</div>
      </div>
      <button class="btn btn-ghost" style="font-size:0.75rem;padding:4px 8px" data-slot="${
        s.slot
