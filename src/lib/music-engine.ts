...
class MusicEngine {
  ...
  private synths: (Tone.Synth | Tone.FMSynth | Tone.AMSynth | Tone.PluckSynth | Tone.MembraneSynth | Tone.PolySynth)[] = []
  ...
  async init(style: Style, analysis: Analysis) {
    ...
    const melodyPart = new Tone.Sequence((time, note: any) => {
    ...
    if (music.voices >= 2) {
      ...
      const bassPart = new Tone.Sequence((time, note: any) => {
        if (note) (bassSynth as any).triggerAttackRelease(note.pitch, note.dur, time, note.vel * 0.7)
      }, bassLine, '4n')
      bassPart.loop = true
    ...
    if (music.voices >= 3 || style === 'horror') {
      ...
      const padPart = new Tone.Sequence((time, chord: any) => {
        if (chord) chord.forEach((n: string) => (padSynth as any).triggerAttackRelease(n, '2n', time, 0.3))
      }, chords, '2n')
      padPart.loop = true
...
function buildNotePool(root: string, scale: number[], baseOctave: number, octaveRange: [number, number]): string[] {
...
function buildMelody(analysis: Analysis, notePool: string[], rng: SeededRNG, style: Style) {
  ...
  for (let i = 0; i < length; i++) {
    if (rng2.next() < analysis.fixRatio * 0.3) { melody.push(null); continue }
...
function buildBassLine(analysis: Analysis, root: string, scale: number[], octave: number, rng: SeededRNG) {
  const rootIdx = ROOTS.indexOf(root)
...
function buildPercussion(rhythm: number[], style: Style, destination: Tone.Volume): Tone.Sequence | null {
  try {
