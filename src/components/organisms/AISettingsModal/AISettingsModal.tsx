...
export function AISettingsModal({ onClose }: AISettingsModalProps) {
  const { aiMode, localModel, apiBaseUrl, apiKey, apiModel, githubToken, rateLimit, saveSettings } = useStore()
  ...
  const handleSave = () => {
    saveSettings({ aiMode: mode, localModel: lModel, apiBaseUrl: aBaseUrl, apiKey: aKey, apiModel: aModel, githubToken: ghToken })
      ...
      <div className={styles.content}>
        ...
        <div className={styles.section}>
          ...
          <div className={styles.radioGroup}>
            <label className={styles.radioOption}>
              <input type="radio" name="ai-mode" value="local" checked={mode === 'local'} onChange={() => setMode('local')} />
              <span>🖥 Local Model (runs in browser, no API key needed)</span>
            </label>
            <label className={styles.radioOption}>
              <input type="radio" name="ai-mode" value="api" checked={mode === 'api'} onChange={() => setMode('api')} />
        ...
        {mode === 'local' && (
          <div className={styles.section}>
            ...
            <select className={styles.select} value={lModel} onChange={e => setLModel(e.target.value)}>
              <option value="Phi-3-mini-4k-instruct-q4f16_1-MLC">Phi-3 Mini 4K (recommended, ~2GB)</option>
              <option value="gemma-2-2b-it-q4f16_1-MLC">Gemma 2 2B (~1.5GB)</option>
              <option value="SmolLM2-1.7B-Instruct-q4f16_1-MLC">SmolLM2 1.7B (~1GB, fastest)</option>
            </select>
            <p className={styles.hint}>⚠ Model will be downloaded to your browser cache on first use.</p>
          </div>
        )}
        ...
        {mode === 'api' && (
          <div className={styles.section}>
            ...
            <Input value={aBaseUrl} onChange={e => setABaseUrl(e.target.value)} placeholder="https://api.openai.com/v1" />
            <label className={styles.label} style={{ marginTop: '12px' }}>API Key</label>
            <Input type="password" value={aKey} onChange={e => setAKey(e.target.value)} placeholder="sk-..." />
            <label className={styles.label} style={{ marginTop: '12px' }}>Model Name</label>
            <Input value={aModel} onChange={e => setAModel(e.target.value)} placeholder="gpt-4o-mini" />
          </div>
        )}
        ...
        <div className={styles.section}>
          <label className={styles.label}>
            ...
            <a href="https://github.com/settings/tokens?type=beta" target="_blank" rel="noopener noreferrer"
              style={{ color: 'var(--color-accent)', fontSize: '0.75rem' }}>(Get token)</a>
          </label>
